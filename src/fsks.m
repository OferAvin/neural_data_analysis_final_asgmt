...
% is five times the range. lastly it runs the Kolmogorov-Smirnov test for all distribitions
    ...
    hist1 = cellfun(@(x,y,z) histcounts(x,linspace(y,z,(z-y)*5),'no','pr'),...
        celFeat1,minVal,maxVal,'UniformOutput',false);  %calculate distribution for 1st class
    hist2 = cellfun(@(x,y,z) histcounts(x,linspace(y,z,(z-y)*5),'no','pr'),...
